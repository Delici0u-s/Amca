...
def run(
    ...
    if build_dir_name is None:
        logger.error(f"[clean] '{build_var_name}' is missing or misconfigured in {meson_file}")
        return False
    ...
    if install_dir_name is None:
        logger.error(f"[clean] '{install_var_name}' is missing or misconfigured in {meson_file}")
        return False
    if executable_name is None:
        logger.error(f"[clean] '{executable_var_name}' is missing or misconfigured in {meson_file}")
        return False
    ...
    if meson_build_dir.exists():
        if opts.dry_run:
            logger.log(f"[dry-run] rm -rf {meson_build_dir}")
        else:
            logger.log(f"Removing build dir: {meson_build_dir}")
            shutil.rmtree(meson_build_dir)
            logger.success("Build dir removed.")
    else:
        logger.warn(f"[clean] Build dir not found, skipping: {meson_build_dir}")

    # --- Clean installed executable ---
    if executable_path.exists():
        if opts.dry_run:
            logger.log(f"[dry-run] rm {executable_path}")
        else:
            logger.log(f"Removing executable: {executable_path}")
            executable_path.unlink()
    else:
        logger.warn(f"[clean] Executable not found, skipping: {executable_path}")
    ...
    if executable_dir.exists() and executable_dir != meson_build_dir:
        if not any(executable_dir.iterdir()):
            if opts.dry_run:
                logger.log(f"[dry-run] rmdir {executable_dir}")
            else:
                logger.log(f"Removing empty install dir: {executable_dir}")
