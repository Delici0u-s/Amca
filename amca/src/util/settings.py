...
class Settings:
  ...
  def save(self) -> None:
    ...
    with self._lock:
      ...
      temp = tempfile.NamedTemporaryFile(mode, delete=False, dir=str(self._path.parent))
      try:
...
if __name__ == "__main__":
  s : Settings = Settings(f"{Path(__file__).parent / "settings_test/settings"}", backend="json")
  s.load()
  s.default("Last_Session.Input", "No previous input") # if ommitted s.get would return None
